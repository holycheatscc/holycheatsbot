...
module.exports.run = async (bot, message, args) => {
    ...
    if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Du hast nicht auszureichende Rechte!").then(msg => msg.delete(10000));
    let wUser = message.guild.member(message.mentions.users.first()) || message.guild.members.get(args[0])
    if(!wUser) return message.reply("Invalid user.").then(msg => msg.delete(5000));
    if(wUser.hasPermission("ADMINISTRATOR")) return message.reply("Can't touch them.").then(msg => msg.delete(5000));
